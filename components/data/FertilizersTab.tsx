    ...
    <div className="space-y-6">
      ...
      <div>
        <h3 className="font-semibold text-slate-700 mb-3 flex items-center gap-2">
          ...
          <span className="font-(family-name:--font-hind) text-slate-400 font-normal text-sm">
            - সার
          </span>
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {fertilizers.map((f) => (
            <div
              key={f.name_en}
              className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col gap-2 hover:shadow-sm transition-shadow"
            >
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="font-semibold text-slate-800">
                    {lang === "en" ? f.name_en : f.name_bn}
                  </div>
                  <div
                    className={`text-sm text-slate-500 ${lang === "en" ? "font-(family-name:--font-hind)" : ""}`}
                  >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {nutrients.map((n) => (
            <div
              key={n.name_en}
              className="bg-white border border-slate-200 rounded-lg p-3 text-center hover:shadow-sm transition-shadow"
            >
              <div className="font-medium text-slate-800 text-sm">
                {lang === "en" ? n.name_en : n.name_bn}
              </div>
              <div
                className={`text-xs text-slate-400 mt-0.5 ${lang === "en" ? "font-(family-name:--font-hind)" : ""}`}
              >
